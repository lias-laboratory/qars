...
public class JSONResultSet {
	 ...
	 * @return
	 */
	public static boolean bindingsInclude(JsonArray bindings2, JsonObject binding) {
	 ...
	 * @return
	 */
	public static boolean bindingsInclude(JsonObject jsonObject, JsonObject binding) {
		for (String key : jsonObject.keySet()) {
			if (binding.keySet().contains(key)) {
				if (!jsonObject.get(key).getAsObject().get("value").toString()
						.equals(binding.get(key).getAsObject().get("value").toString())) {
					return false;
				}
			}
		}
