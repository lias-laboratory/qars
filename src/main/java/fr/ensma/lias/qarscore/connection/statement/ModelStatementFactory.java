...
import fr.ensma.lias.qarscore.connection.statement.implementation.JenaModelStatement;
import fr.ensma.lias.qarscore.connection.statement.implementation.SesameModelStatement;

/**
 * @author Geraud FOKOU
 */
public class ModelStatementFactory {


    public static ModelStatement createQueryStatement (Session session){
	
	if((session instanceof SessionJenaSDB) || (session instanceof SessionJenaTDB)){
	    return new JenaModelStatement((JenaSession) session);
	}
	if(session instanceof SesameSession) {
	    return new SesameModelStatement((SesameSession) session);
	}
	return null;
    }
