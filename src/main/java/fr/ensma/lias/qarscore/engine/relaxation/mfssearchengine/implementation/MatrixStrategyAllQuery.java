...
public class MatrixStrategyAllQuery extends MatrixStrategy {
	 ...
	 */
	protected MatrixStrategyAllQuery(Session s, CQuery conjunctiveQuery, int expected_answers_number) {
		...
		values = new RoaringBitmap[CURRENT_CONJUNCTIVE_QUERY.getElementList().size()];
	...
	private void initializeMatrix() {
		...
		for (int i = 1; i <= number_Element; i++) {
			...
			JSONResultSet result_set = SESSION.executeSelectQuery(current_query.toString());
			...
			while (result_set.hasNext()) {
				...
				int[] listMapping = new int[CURRENT_CONJUNCTIVE_QUERY.getMentionedQueryVarNames().size()];

				for (int j = 1; j <= CURRENT_CONJUNCTIVE_QUERY.getMentionedQueryVarNames().size(); j++) {
					String val = result_set.getString(CURRENT_CONJUNCTIVE_QUERY.getMentionedQueryVarNames().get(j - 1));
					Integer intVal = null;
					if (val == null)
						intVal = 0;
					else {
				...
				for (MappingResult store_mapping : actualMappings.keySet()) {
					...
					if (result_mapping.isCompatible(store_mapping)) {
						MappingResult unionMapping = result_mapping.union(store_mapping);
	...
	public boolean hasLeastKAnswers() {
		...
		for (CElement element : CURRENT_CONJUNCTIVE_QUERY.getElementList()) {
			int index = CURRENT_CONJUNCTIVE_QUERY.getElementList().indexOf(element);
			if (index != -1) {
	...
	public boolean isMFS() {
		...
		for (CElement element : CURRENT_CONJUNCTIVE_QUERY.getElementList()) {
			...
			for (i = i + 1; i < CURRENT_CONJUNCTIVE_QUERY.getElementList().size(); i++) {
				if (CURRENT_CONJUNCTIVE_QUERY.getElementList().get(0) != element) {
