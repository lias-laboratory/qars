...
public class XSSFineGrainedRelaxation extends AbstractXSSRelaxationStrategy {
	...
	protected void initilizationXSSComp(CQuery query, Session s, boolean optimization) {
...
//	}
		for (int i = 0; i < xss_to_relax_queries.length; i++) {
			xss_complement_queries[i] = query.difference(xss_to_relax_queries[i]);
			...
			comp_xss_query_index[i] = new int[xss_complement_queries[i].getElementList().size()];
			int[] relaxation_index_comp_xss = new int[xss_complement_queries[i].getElementList().size()];
			int[] relaxation_limit_comp_xss = new int[xss_complement_queries[i].getElementList().size()];
			...
			for (int j = 0; j < query_to_relax.getElementList().size(); j++) {
				...
				int index = xss_complement_queries[i].getElementList().indexOf(element);
				if (index != -1) {
					...
					similarity_comp_xss[i] = similarity_comp_xss[i]
							* sim_measure.suppressTripleMeasure(query.getElementList().get(j));
				}
			}
			relaxed_comp_xss[i] = CQueryFactory.createCQuery(elt_relaxed_query, query_to_relax.getSelectedQueryVar());
			relaxed_comp_queries[i]
					.add(new GraphRelaxationIndex(relaxation_index_comp_xss, relaxation_limit_comp_xss, !optimization));
			GraphRelaxationIndex relax_graph_node = relaxed_queries[i].remove(0);
			for (int j = 0; j < relax_graph_node.getChild_elt().length; j++) {
				this.insert_relaxation_graph_node(relax_graph_node.getChild_elt()[j], i);
			}
		}
	}
	 ...
	 * @param optimization
	 */
	public XSSFineGrainedRelaxation(CQuery query, Session s, boolean optimization) {
	...
	public CQuery next() {
		...
		if (!relaxed_comp_queries[pos].isEmpty()) {
			GraphRelaxationIndex relaxed_comp_node = relaxed_comp_queries[pos].remove(0);
			for (int j = 0; j < relaxed_comp_node.getChild_elt().length; j++) {
				this.insert_relaxation_comp_graph_node(relaxed_comp_node.getChild_elt()[j], pos);
			}
			...
			for (int j = 0; j < relax_graph_node.getChild_elt().length; j++) {
				this.insert_relaxation_graph_node(relax_graph_node.getChild_elt()[j], pos);
			}
	...
	protected CQuery getQuery(GraphRelaxationIndex index_query, int xss_index) {
		...
		for (int i = 0; i < index_query.getElement_index().length; i++) {
			...
			elt_relaxed_query.add(getRelaxedElement(index_in_query, index_query.getElement_index()[i]));

			this.current_similarity = this.current_similarity
					* relaxation_of_element[index_in_query][index_query.getElement_index()[i]].getSimilarity();

			if (index_query.getElement_index()[i] == 0) {
				current_satisfactory = current_satisfactory + 1;
			}

			this.current_level.add(
					relaxation_of_element[index_in_query][index_query.getElement_index()[i]].getRelaxation_level());
		}

		this.current_satisfactory = current_satisfactory / query_to_relax.getElementList().size();
...
//	return CQueryFactory.createCQuery(elt_relaxed_query);
	...
	protected CQuery getCompQuery(GraphRelaxationIndex relaxed_comp_node, int comp_xss_index) {

		List<CElement> elt_relaxed_query = new ArrayList<CElement>();
		elt_relaxed_query.addAll(xss_to_relax_queries[comp_xss_index].getElementList());
		this.current_similarity = 1.0;
		this.current_satisfactory = xss_to_relax_queries[comp_xss_index].getElementList().size();
		this.current_level = new ArrayList<int[]>();

		for (int i = 0; i < relaxed_comp_node.getElement_index().length; i++) {
			int index_in_query = comp_xss_query_index[comp_xss_index][i];
			elt_relaxed_query.add(getRelaxedElement(index_in_query, relaxed_comp_node.getElement_index()[i]));

			this.current_similarity = this.current_similarity
					* relaxation_of_element[index_in_query][relaxed_comp_node.getElement_index()[i]].getSimilarity();

			if (relaxed_comp_node.getElement_index()[i] == 0) {
				current_satisfactory = current_satisfactory + 1;
			}

			this.current_level.add(relaxation_of_element[index_in_query][relaxed_comp_node.getElement_index()[i]]
					.getRelaxation_level());
		}
	...
	protected int getPositionRelaxGraph() {
		...
		for (int i = 0; i < relaxed_comp_queries.length; i++) {
			if (!relaxed_comp_queries[i].isEmpty()) {
				double current_sim = this.similarity_comp_graph_index(relaxed_comp_queries[i].get(0), i);
				...
				if (!relaxed_queries[i].isEmpty()) {
					double current_sim = this.similarity_graph_index(relaxed_queries[i].get(0), i);
					if (current_sim > sim_pos) {
						pos = i;
						sim_pos = current_sim;
					}
	...
	protected double similarity_comp_graph_index(GraphRelaxationIndex relax_comp, int comp_xss_index) {
		...
		for (int i = 0; i < relax_comp.getElement_index().length; i++) {
			...
			current_comp_similarity = current_comp_similarity
					* relaxation_of_element[index_in_query][relax_comp.getElement_index()[i]].getSimilarity();
		}
		return current_comp_similarity;
	}

	protected void insert_relaxation_comp_graph_node(GraphRelaxationIndex child, int comp_xss_index) {

		double current_child_similarity = this.similarity_comp_graph_index(child, comp_xss_index);
		int pos = 0;
		int index = relaxed_comp_queries[comp_xss_index].size() - 1;
		boolean found_pos = false;
		while ((!found_pos) && (0 <= index)) {
			double index_sim = 1.0;
			GraphRelaxationIndex elt_index = relaxed_comp_queries[comp_xss_index].get(index);
			index_sim = this.similarity_comp_graph_index(elt_index, comp_xss_index);
...
//	    }
	...
	protected double similarity_graph_index(GraphRelaxationIndex child, int xss_index) {
		...
		for (int i = 0; i < child.getElement_index().length; i++) {
			...
			current_child_similarity = current_child_similarity
					* relaxation_of_element[index_in_query][child.getElement_index()[i]].getSimilarity();
		}
