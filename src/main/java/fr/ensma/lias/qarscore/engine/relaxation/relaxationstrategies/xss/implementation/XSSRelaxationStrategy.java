...
//	}
		for (int i = 0; i < xss_to_relax_queries.length; i++) {
			...
			CQuery comp_xss = query_to_relax.difference(xss_to_relax_queries[i]);
			...
			for (int j = 0; j < comp_xss.getElementList().size(); j++) {
				elt_relaxed_query.add(comp_xss.getElementList().get(j).supress_all_concrete());
				similarity_comp_xss[i] = similarity_comp_xss[i]
						* sim_measure.suppressTripleMeasure(comp_xss.getElementList().get(j));
			}
	...
	protected double similarity_graph_index(GraphRelaxationIndex child, int xss_index) {
		...
		for (int i = 0; i < child.getElement_index().length; i++) {
			...
			current_child_similarity = current_child_similarity
					* relaxation_of_element[index_in_query][relax_rank].getSimilarity();
		}
