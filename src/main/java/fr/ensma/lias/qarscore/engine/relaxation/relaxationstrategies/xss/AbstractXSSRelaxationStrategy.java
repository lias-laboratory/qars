 ...
 * @author Geraud FOKOU
 */
public abstract class AbstractXSSRelaxationStrategy extends AbstractRelaxationStrategy {
	...
	protected void initilizationXSS(CQuery query, Session s, boolean optimization) {
		...
		relaxation_of_element = new NodeRelaxed[query_to_relax.getElementList().size()][];
		int[] relaxation_limit_index = new int[query_to_relax.getElementList().size()];
		...
		for (int i = 0; i < query_to_relax.getElementList().size(); i++) {
			...
			relaxation_of_element[i] = (new TripleRelaxation(element, session, TripleRelaxation.SIM_ORDER))
					.get_relaxed_node_list();
...
//	} else {
		for (int i = 0; i < xss_queries.size(); i++) {
			...
			xss_query_index[i] = new int[xss_queries.get(i).getElementList().size()];
			int[] relaxation_index_xss = new int[xss_queries.get(i).getElementList().size()];
			int[] relaxation_limit_xss = new int[xss_queries.get(i).getElementList().size()];

			for (int j = 0; j < query_to_relax.getElementList().size(); j++) {
				CElement element = query_to_relax.getElementList().get(j);
				int index = xss_to_relax_queries[i].getElementList().indexOf(element);
				if (index != -1) {
			...
			relaxed_queries[i].add(new GraphRelaxationIndex(relaxation_index_xss, relaxation_limit_xss, !optimization));
		}
//	}
	...
	public AbstractXSSRelaxationStrategy(CQuery query, Session s) {
		...
		xss_finders = StrategyFactory.getLatticeStrategy(s, query, MFS_LIMIT_ANSWERS);
	 ...
	 * Abstract constructor
	 */
	protected AbstractXSSRelaxationStrategy(CQuery query, Session s, boolean optimization) {

		xss_finders = StrategyFactory.getLatticeStrategy(s, query, MFS_LIMIT_ANSWERS, optimization);
		this.initilizationXSS(query, s, true);
	}
	...
	protected CElement getRelaxedElement(int num_triple, int relaxation_rank) {
		...
		CElement relax_element = CElement.createCTriple(query_to_relax.getElementList().get(0).getElement());
		relax_element = relax_element.replace_subject(relaxation_of_element[num_triple][relaxation_rank].getNode_1());
		relax_element = relax_element.replace_predicat(relaxation_of_element[num_triple][relaxation_rank].getNode_2());
		relax_element = relax_element.replace_object(relaxation_of_element[num_triple][relaxation_rank].getNode_3());
	 ...
	 * @return
	 */
	protected boolean alreadyRelaxed(GraphRelaxationIndex graphRelaxationIndex, int xss_index) {
	...
	protected abstract double similarity_graph_index(GraphRelaxationIndex child, int xss_index);

	protected void insert_relaxation_graph_node(GraphRelaxationIndex child, int xss_index) {
		...
		while ((!found_pos) && (0 <= index)) {
			...
			GraphRelaxationIndex elt_index = relaxed_queries[xss_index].get(index);
	...
	public CQuery[] getAllRelaxedQueries() {
		...
		for (int j = 0; j < already_relaxed_xss.length; j++) {
			for (int i = 0; i < already_relaxed_xss[j].size(); i++) {
				relaxed_queries[i] = this.getQuery(already_relaxed_xss[j].get(i), j);
			}
		}
