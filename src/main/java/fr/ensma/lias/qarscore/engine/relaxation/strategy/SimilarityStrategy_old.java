 ...
 * @author Geraud FOKOU
 */
public class SimilarityStrategy_old {
     ...
     * 
     */
    public SimilarityStrategy_old(CQuery query, Session s) {
	...
	if (node.isURI()) {
	    if (((JenaSession)session).getOntology().getOntClass(node.getURI()) != null) {
		Map<CQuery, List<Double>> genqueries = operator_relax
			.generalize(graph_to_relax.getQuery(), node, 1, s);
		...
		Map<CQuery, Double> sibqueries = operator_relax.sibling(
			graph_to_relax.getQuery(), node, s);
	...
	if (((JenaSession)session).getOntology().getOntClass(uri) != null) {
		...
		Map<CQuery, List<Double>> genqueries = operator_relax
			.generalize(current_root.getQuery(),
				NodeFactory.createURI(uri), level, session);
	...
	if (((JenaSession)session).getOntology().getOntClass(uri) != null) {
	    ...
	    for (RelaxationTree current_root : current_roots) {
		Map<CQuery, Double> sibqueries = operator_relax.sibling(
			current_root.getQuery(), NodeFactory.createURI(uri), session);
		for (CQuery q : sibqueries.keySet()) {
