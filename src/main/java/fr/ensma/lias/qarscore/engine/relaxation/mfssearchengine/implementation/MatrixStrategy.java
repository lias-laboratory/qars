 ...
 * @author Geraud FOKOU
 */
public abstract class MatrixStrategy implements MFSSearch {
	...
	/**
	 * Boolean value for a MappingResult of each triple pattern Column storage of
	 * boolean
	 */
	 ...
	 */
	protected MatrixStrategy(Session s, CQuery conjunctiveQuery, int expected_answers_number) {
		...
		for (int i = 2; i <= actualQuery.getElementList().size(); i++) {
			...
			for (ICombinatoricsVector<Integer> combination : gen) {
				...
				for (int j = 1; j < listeTi.size(); j++) {
					bitRes = RoaringBitmap.and(bitRes, getBitVector(listeTi.get(i)));
				}
				if (bitRes.isEmpty()) {
					List<CElement> causes = new ArrayList<CElement>();
		...
		if (notEmptyCElement.size() > 1) {
			...
			for (int i = 2; i <= actualQuery.getElementList().size(); i++) {
				...
				for (ICombinatoricsVector<Integer> combination : gen) {
					...
					RoaringBitmap bitRes = getBitVector(listeTi.get(0));
					for (int j = 1; j < listeTi.size(); j++) {
						bitRes = RoaringBitmap.and(bitRes, getBitVector(listeTi.get(j)));
					}
					if (bitRes.isEmpty()) {
						boolean isSuper = false;
						int j = 0;
						while ((j < mfsCombination.size()) && (!isSuper)) {
							isSuper = combination.getVector().containsAll(mfsCombination.get(j).getVector());
							j = j + 1;
						}
						if (!isSuper) {
	...
	public List<CQuery> getAllXSS(CQuery query) {
		...
		Integer[] CElementArray = new Integer[actualQuery.getElementList().size()];
		for (int i = 0; i < actualQuery.getElementList().size(); i++) {
			CElementArray[i] = i + 1;
		}

		ICombinatoricsVector<Integer> initialVector = Factory.createVector(CElementArray);
			...
			for (ICombinatoricsVector<Integer> combination : gen) {
				...
				if (!bitRes.isEmpty()) {
					...
					while ((j < xssCombination.size()) && (!isSubSet)) {
						isSubSet = xssCombination.get(j).getVector().containsAll(combination.getVector());
						j = j + 1;
					}
					if (!isSubSet) {
