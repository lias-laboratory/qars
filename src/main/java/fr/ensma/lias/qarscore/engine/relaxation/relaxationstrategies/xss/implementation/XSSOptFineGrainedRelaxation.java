...
//			    .toConstructQuery()));
			comp_session[i] = SessionFactory.getModelSession(
					((JenaTDBSession) session).executeModelConstructQuery(xss_to_relax_queries[i].toConstructQuery()));
	 ...
	 * @param optimization
	 */
	public XSSOptFineGrainedRelaxation(CQuery query, Session s, boolean optimization) {
		super(query, s, optimization);
		comp_session = new Session[xss_to_relax_queries.length];
		duration_computation_view = System.currentTimeMillis();
		for (int i = 0; i < xss_to_relax_queries.length; i++) {
			comp_session[i] = SessionFactory
					.getModelSession(session.executeConstructQuery(xss_to_relax_queries[i].toConstructQuery()));
		}
		duration_computation_view = System.currentTimeMillis() - duration_computation_view;
	}
