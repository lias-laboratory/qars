 ...
 * @author Geraud FOKOU
 */
public class MFSBaseOptimizedRelaxationStrategy extends MFSBaseRelaxationStrategy {
	 ...
	 * @param optimization
	 */
	public MFSBaseOptimizedRelaxationStrategy(CQuery query, Session s, boolean optimization) {
	...
	protected GraphRelaxationIndex getLeastRelaxedAncestor(List<GraphRelaxationIndex> already_failed_relaxed_queries,
		...
		while ((0 <= i) && (!is_relaxation)) {
			is_relaxation = true;
			int j = 0;
			while ((j < relax_graph_node.getElement_index().length) && (is_relaxation)) {
				CElement relax_elt = getRelaxedElement(j, relax_graph_node.getElement_index()[j]);
				CElement father_elt = getRelaxedElement(j, already_failed_relaxed_queries.get(i).getElement_index()[j]);
				is_relaxation = is_relaxation && TripleRelaxation.is_relaxation(relax_elt, father_elt, session);
				j = j + 1;
			}
			i = i - 1;
		}
	 ...
	 * @return
	 */
	protected boolean is_in_mfs_degree(List<int[]> mfs_degrees, int[] current_mfs_degree) {
		...
		while (0 <= j) {
			...
			while ((k < current_mfs_degree.length) && (is_in)) {
				is_in = is_in && (mfs_degrees.get(j)[k] == current_mfs_degree[k]);
				k = k + 1;
			}
			if (is_in) {
				return true;
	...
	/**
	 * Check if relaxed MFS are still MFS and add them into the list of current MFS
	 * Return the MFS which are been repaired
	 * 
	 * @param relax_graph_node
	 ...
	 * @return
	 */
	protected List<Integer> add_relaxed_mfs(GraphRelaxationIndex relax_graph_node, List<Integer> current_mfs_relaxed,
			List<int[]> degree_mfs_relaxed) {
		...
		for (int i = 0; i < current_mfs_relaxed.size(); i++) {
			List<CElement> updated_mfs_query = get_element_list(current_relax_query,
			...
			Query temp_query = mfs_query.getSPARQLQuery();
			temp_query.setLimit(MFS_LIMIT_ANSWERS);
			long begin = System.currentTimeMillis();
			if (session.getResultSize(temp_query.toString()) < MFS_LIMIT_ANSWERS) {
				mfs_degree_by_index.get(current_mfs_relaxed.get(i)).add(degree_mfs_relaxed.get(i));
				has_mfs = true;
				logger.info("New MFS " + mfs_query.toString());
			} else {
				repaired_mfs.add(current_mfs_relaxed.get(i));
				repaire_mfs_degree_by_index.get(current_mfs_relaxed.get(i)).add(degree_mfs_relaxed.get(i));
