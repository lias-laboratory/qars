...
//    protected GraphRelaxationIndex relaxed_graph;
	...
	public AbstractMFSRelaxationStrategy(CQuery query, Session s) {
		...
		relaxed_queries.add(new GraphRelaxationIndex(relaxation_index, relaxation_limit_index));
	}
	...
	protected AbstractMFSRelaxationStrategy(CQuery query, Session s, boolean optimization) {
		...
		relaxed_queries.add(new GraphRelaxationIndex(relaxation_index, relaxation_limit_index, !optimization));
	}
	...
	public CQuery[] getAllRelaxedQueries() {
		...
		for (int i = 0; i < already_relaxed_queries.size(); i++) {
			relaxed_queries[i] = this.getQuery(already_relaxed_queries.get(i));
		}
	...
	protected CQuery getQuery(GraphRelaxationIndex index_query) {
		...
		for (int i = 0; i < index_query.getElement_index().length; i++) {
			...
			elt_relaxed_query.add(getRelaxedElement(i, index_query.getElement_index()[i]));

			this.current_similarity = this.current_similarity
					* relaxation_of_element[i][index_query.getElement_index()[i]].getSimilarity();

			if (index_query.getElement_index()[i] == 0) {
				current_satisfactory = current_satisfactory + 1;
			}

			this.current_level.add(relaxation_of_element[i][index_query.getElement_index()[i]].getRelaxation_level());
		}
	...
	protected void insert_relaxation_graph_node(GraphRelaxationIndex child) {
		...
		for (int i = 0; i < child.getElement_index().length; i++) {
			...
			current_child_similarity = current_child_similarity
					* relaxation_of_element[i][child.getElement_index()[i]].getSimilarity();
		...
		while ((!found_pos) && (0 <= index)) {
			...
			for (int i = 0; i < elt_index.getElement_index().length; i++) {
				index_sim = index_sim * relaxation_of_element[i][elt_index.getElement_index()[i]].getSimilarity();
			}
