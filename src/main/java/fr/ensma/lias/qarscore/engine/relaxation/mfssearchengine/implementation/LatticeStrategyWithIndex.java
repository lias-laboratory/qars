...
public class LatticeStrategyWithIndex extends AbstractLatticeStrategy {
	 ...
	 * @return
	 */
	protected static LatticeStrategyWithIndex getLatticeStrategyWithIndex(Session s, CQuery query, int answers) {
	...
	public boolean hasLeastKAnswers(CQuery query) {
		...
		for (CQuery a_connex_query : queries) {
			...
			if (nbSolution >= NUMBER_OF_EXPECTED_ANSWERS) {
				logger.info(
						"Execution of : " + a_connex_query.getQueryLabel() + "                           Once Succes");
			} else {
				logger.info(
						"Execution of : " + a_connex_query.getQueryLabel() + "                           Once Echec");
				if (queries.size() != 1) {
					logger.info("*******************End Execution of query with cartesian product: "
							+ query.getQueryLabel() + "**********************************");
				}
