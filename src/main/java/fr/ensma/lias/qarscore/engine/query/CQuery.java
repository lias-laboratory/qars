...
public class CQuery implements Comparable<CQuery> {
	 ...
	 */
	private CQuery(List<CElement> elt, List<ElementGroup> gr, List<Node> selectedVar) {
		elementList = elt;
		groupList = gr;
	 ...
	 * @return
	 */
	protected static CQuery createCQuery(List<CElement> elt, List<ElementGroup> gr, List<Node> selectedVar) {
	...
	public List<String> getMentionedQueryVarNames() {
		...
		for (int i = 0; i < this.getElementList().size(); i++) {
			tempvarnames.addAll(this.getElementList().get(i).getMentionnedVarsNames());
			// tempvarnames.retainAll(varnames);
			varnames.removeAll(tempvarnames);
			varnames.addAll(this.getElementList().get(i).getMentionnedVarsNames());
			tempvarnames.clear();
		}
		return varnames;
	}
	...
	/**
	 * true if the query is valid i.e there are at least one triple element and all
	 * the filter element use variable present in at least one triplet
	 * 
	 * @return
	 */
	public boolean isValidQuery() {
		...
		for (int i = 0; i < this.getElementList().size(); i++) {
			...
			if (this.getElementList().get(i).getElement() instanceof ElementPathBlock) {
				...
				tempVarnames.addAll(this.getElementList().get(i).getMentionnedVar());

				tempVarnames.retainAll(triplePattern);
				triplePattern.removeAll(tempVarnames);
				triplePattern.addAll(this.getElementList().get(i).getMentionnedVar());
				tempVarnames.clear();
			}
		}
		...
		while ((i < this.getElementList().size()) && (isValid)) {
			...
			if (this.getElementList().get(i).getElement() instanceof ElementFilter) {
				isValid = isValid && triplePattern.containsAll(this.getElementList().get(i).getMentionnedVar());
			}
			i = i + 1;
		}
	...
	public static CQuery getSuperClassQuery(Node classNode) {
		...
		String subclassOf = rdf_prefix + "SELECT ?superclass  " + "WHERE {?subclass rdfs:subClassOf  ?superclass }";
		CQuery subQuery = CQueryFactory.createCQuery(QueryFactory.create(subclassOf));
	...
	public static CQuery getSubClassQuery(Node classNode) {
		...
		String rdf_prefix = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>";
		String subclassOf = rdf_prefix + "SELECT ?subclass  " + "WHERE {?subClasses rdfs:subClassOf  ?superclass }";
		CQuery subQuery = CQueryFactory.createCQuery(QueryFactory.create(subclassOf));
		CElement new_Element = subQuery.getElementList().get(0).replace_object(classNode);
		subQuery.getElementList().remove(0);
		subQuery.getElementList().add(new_Element);
	...
	/**
	 * Add triple of a query where all the concrete values are changed into
	 * variables
	 * 
	 * @param query
	 * @param xss
	...
	/**
	 * Transform a select query to a construct query with all the triple in the
	 * where clause as template of the construct clause
	 * 
	 * @param cQuery
	 * @return
	 */
